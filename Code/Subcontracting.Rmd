...
scale_color_wrapped <- scale_color_discrete(labels = ~ stringr::str_wrap(.x, 28))
scale_fill_wrapped  <- scale_fill_discrete(labels  = ~ stringr::str_wrap(.x, 28))
...
if (exists("subcontract") && !exists("subcontracts")) {
  subcontracts <- subcontract
}
stopifnot(exists("subcontracts"))

if ("prime_id" %in% names(subcontracts)) {
  sub <- subcontracts %>%
    group_by(prime_id) %>%
    slice_head(n = 1) %>%
    ungroup()
} else if ("prime_id_num" %in% names(subcontracts)) {
  sub <- subcontracts %>%
    group_by(prime_id_num) %>%
    slice_head(n = 1) %>%
    ungroup()
} else {
  sub <- subcontracts %>% distinct(.keep_all = TRUE)
}

key_candidate <- intersect(c("prime_id_num","prime_id"), names(subcontracts))
stopifnot(length(key_candidate) >= 1)
best_key <- key_candidate[1]

subcontracts <- subcontracts %>% mutate(obs_id = as.character(.data[[best_key]]))
sub <- sub %>% mutate(obs_id = as.character(.data[[best_key]]))

coerce_date <- function(x) suppressWarnings(as.Date(x))
if ("prime_date" %in% names(subcontracts)) {
  subcontracts <- subcontracts %>% mutate(prime_date = coerce_date(prime_date),
                                          year = lubridate::year(prime_date))
}
if ("prime_date" %in% names(sub)) {
  sub <- sub %>% mutate(prime_date = coerce_date(prime_date),
                        year = lubridate::year(prime_date))
}

if (!"nom_g" %in% names(subcontracts)) {
  valcand <- intersect(c("obligated_amount","action_obligation","amount","value"), names(subcontracts))
  if (length(valcand) == 1) {
    subcontracts <- subcontracts %>% rename(nom_g = !!valcand)
  }
}
if (!"nom_g" %in% names(sub) && "nom_g" %in% names(subcontracts)) {
  keep_cols <- intersect(c("obs_id","nom_g"), names(subcontracts))
  sub <- sub %>% select(-any_of("nom_g")) %>% left_join(subcontracts %>% select(any_of(keep_cols)), by="obs_id")
}

if (!"product_or_service_code_descript" %in% names(subcontracts)) {
  alt_psc <- intersect(c("product_or_service_code_description","psc_description","psc_title"), names(subcontracts))
  if (length(alt_psc) == 1) {
    subcontracts <- subcontracts %>%
      rename(product_or_service_code_descript = !!alt_psc)
  }
}

if (!"naics_code" %in% names(sub)) {
  if ("naics_code" %in% names(subcontracts)) {
    sub <- sub %>% select(-any_of("naics_code")) %>%
      left_join(subcontracts %>% select(obs_id, naics_code), by = "obs_id")
  }
}
if (!"year" %in% names(sub) && "year" %in% names(subcontracts)) {
  sub <- sub %>% left_join(subcontracts %>% select(obs_id, year), by = "obs_id")
}
...
std_one <- attach_naics_titles_2022(std_one, code_col = "naics_2022", key_df = codes22_key, levels = c(6,3,2))

naics_prior_2022 <- build_naics_prior_2022(std, psc_col = "product_or_service_code_descript",
                                           naics_col = "naics_2022", weight_col = "weight")

std %>% dplyr::count(year, map_path) %>% arrange(year, map_path)
...
naics_key6 <- std_one %>% distinct(naics_2022, naics6_title)
...
summary_data_all <- std_one %>%
  ...
  left_join(naics_key6, by = "naics_2022", relationship = "many-to-one")
