...
out_outputs <- here("Data", "SAM_ER", "Priors")
...
subcontracts %>% 
  mutate(year_sub = lubridate::year(lubridate::date(sub_date))) %>% 
  group_by(year_sub) %>% 
  summarise( 
...
summary(core2$`UNIQUE ENTITY ID`)
core2$`UNIQUE ENTITY ID`[1:5]
try <-core2 %>%
  mutate(
    UEI_NA = if_else(!is.na(`UNIQUE ENTITY ID`) & `UNIQUE ENTITY ID` == "No longer available", 1L, 0L)
  )%>% 
  group_by(year) %>% 
  summarise(
   mean_na = mean(UEI_NA)
...
# NAICS6: entropy vs frequency
ggplot(prior6$summary, aes(x = n, y = entropy)) +
  geom_point(alpha = 0.6) +
  geom_smooth(se = FALSE, color = "blue") +
  labs(title = "Entropy vs number of PSCs per NAICS6",
       x = "Number of distinct PSCs linked to NAICS6",
       y = "Entropy of PSC distribution") +
  theme_minimal()

# NAICS4: entropy vs frequency
ggplot(priors4$summary, aes(x = n_psc, y = entropy)) +
  geom_point(alpha = 0.6) +
  geom_smooth(se = FALSE, color = "red") +
  labs(title = "Entropy vs number of PSCs per NAICS4",
       x = "Number of distinct PSCs linked to NAICS4",
       y = "Entropy of PSC distribution") +
  theme_minimal()
...
subcontracts %>%
  ...
  labs(
    title = "Share of Subcontractors Missing NAICS Over Time",
...
# add frequency to priors6 summary
priors6_freq <- priors6$prior %>%
  group_by(NAICS) %>%
  summarise(total_n = sum(n), .groups = "drop") %>%
  inner_join(priors6$summary, by = "NAICS")

ggplot(priors6_freq, aes(x = total_n, y = entropy)) +
  geom_point(alpha = 0.6, color = "steelblue") +
  geom_smooth(se = FALSE, color = "red") +
  scale_x_continuous(labels = scales::comma) +
  labs(
    title = "Entropy vs Frequency of NAICS6",
...
# same for NAICS4
priors4_freq <- priors4$prior %>%
  group_by(NAICS) %>%
  summarise(total_n = sum(n), .groups = "drop") %>%
  inner_join(priors4$summary, by = "NAICS")

ggplot(priors4_freq, aes(x = total_n, y = entropy)) +
  geom_point(alpha = 0.6, color = "steelblue") +
  geom_smooth(se = FALSE, color = "red") +
  scale_x_continuous(labels = scales::comma) +
  labs(
    title = "Entropy vs Frequency of NAICS4",
    x = "Number of entity–PSC pairs (frequency)",
    y = "Entropy of PSC distribution"
  ) +
  theme_minimal()
...
priors6$summary <- priors6$summary %>%
  mutate(entropy_norm = entropy / log(n_psc))

priors4$summary <- priors4$summary %>%
  mutate(entropy_norm = entropy / log(n_psc))


hist(priors6$summar$entropy_norm)
...
# add normalized entropy to summaries
priors6$summary <- priors6$summary %>%
  mutate(entropy_norm = entropy / log(n_psc))
...
# join in total frequency for NAICS6
priors6_freq <- priors6$prior %>%
  group_by(NAICS) %>%
  summarise(total_n = sum(n), .groups = "drop") %>%
  inner_join(priors6$summary, by = "NAICS")

ggplot(priors6_freq, aes(x = total_n, y = entropy_norm)) +
  geom_point(alpha = 0.6, color = "steelblue") +
  geom_smooth(se = FALSE, color = "red") +
  scale_x_continuous(labels = scales::comma) +
  scale_y_continuous(limits = c(0, 1), labels = scales::percent) +
  labs(
    title = "Normalized Entropy vs Frequency of NAICS6",
    x = "Number of entity–PSC pairs (frequency)",
    y = "Normalized entropy (0–1 scale)"
  ) +
  theme_minimal()
...
ggplot(priors4_freq, aes(x = total_n, y = entropy_norm)) +
  geom_point(alpha = 0.6, color = "steelblue") +
  geom_smooth(se = FALSE, color = "red") +
  scale_x_continuous(labels = scales::comma) +
  scale_y_continuous(limits = c(0, 1), labels = scales::percent) +
  labs(
    title = "Normalized Entropy vs Frequency of NAICS4",
    x = "Number of entity–PSC pairs (frequency)",
    y = "Normalized entropy (0–1 scale)"
  ) +
  theme_minimal()
