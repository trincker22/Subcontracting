...
saveRDS(df, here("Data", "dta", "prime_sub.rds"))

subcontract <- df %>%  
  select(prime_id, row_id, recipient_uei, psc_code, prime_date, sub_date, 
         sub_id, subawardee_name, subaward_description, naics6) %>% 
     rename( 
     product_or_service_code_descript = psc_code, 
     naics_code = naics6
     )

 
try <-  subcontract %>%  
   group_by(subawardee_name, subaward_description) %>%  
   slice_head(n = 1)
...
subcontract <- subcontract %>%
  mutate(pair_key = digest(paste0(subawardee_name, "||", subaward_description), algo = "xxhash64"))
...
subcontract <- subcontract %>%
  mutate(
    subawardee_name_c = canon(subawardee_name),
    subaward_description_c = canon(subaward_description)
  )

keys_vec <- paste0(subcontract$subawardee_name_c, "||", subcontract$subaward_description_c)

subcontract$pair_key <- vapply(keys_vec, digest, character(1), algo = "xxhash64", serialize = FALSE)
...
proc <- seed %>%
  mutate()  # your text processing here

out <- subcontract %>%
  left_join(proc, by = "pair_key")

stopifnot(nrow(out) == nrow(subcontract))
