...
acr_tbl <- acr_candidates %>%
  ...
  if (is.null(pr) || !length(pr)) pr <- rep(0.5, length(cand_ix))
  pr[!is.finite(pr)] <- 0
  ...
  expand_one <- function(txt, short_flag = FALSE, serial_flag = FALSE, junky_flag = FALSE) {
    if (is.na(txt) || !nzchar(txt)) {
      return(list(text = txt, acrs_detected = character(0), n = 0L,
    ...
    if (!any(can_mask)) {
      return(list(text = txt, acrs_detected = unique(found$acr), n = 0L,
    ...
    list(
      ...
      acrs_detected = unique(found$acr),   # includes those without candidate rows
      n             = length(replace_ix),
      mean_local    = ifelse(length(replace_ix), mean(P$local[replace_ix], na.rm = TRUE), NA_real_),
      max_local     = suppressWarnings(ifelse(all(!is.finite(P$local)), NA_real_, max(P$local, na.rm = TRUE))),
  ...
  if (identical(mode, "tune")) {
    ...
    expanded_text <- vapply(res_all, function(x) if (is.null(x$text)) NA_character_ else x$text, character(1))
...
proc_dir <- file.path(here(), "Data", "processed")
prev <- readRDS(file.path(proc_dir, "expanded_sample_preview.rds"))
