...
canon <- function(x) x |> replace_na("") |> str_squish() |> str_to_lower()

subcontract <- subcontract %>%
  mutate(
    name_c = canon(subawardee_name),
    desc_c = canon(subaward_description),
    id_comp = if_else(!is.na(recipient_uei) & recipient_uei != "", paste0("uei:", recipient_uei), paste0("name:", name_c)),
    pair_key = md5(paste0(id_comp, "||", desc_c))
  )

subcontract %>% count(pair_key) %>% arrange(desc(n)) %>% head()

subcontracts <- subcontract %>%
  group_by(pair_key) %>%
  slice_head(n = 1) %>%
  ungroup() %>%
  select(pair_key, subawardee_name, subaward_description)

stopifnot(nrow(text_seed) == dplyr::n_distinct(text_seed$pair_key))


row_key_text_dedupe <- subcontract
  
saveRDS(row_key_text_dedupe, here("Data", "dta", "rowkey_text_dedupe.rds"))
  
saveRDS(text_seed, here("Data", "dta", "text_seed.rds"))


subcontracts <- text_seed
