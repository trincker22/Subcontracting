...
txtdf <- txtdf %>% dplyr::ungroup()
stopifnot(nrow(txtdf) == length(expanded_description))

out <- txtdf %>%
  dplyr::mutate(
    expanded_description = .env$expanded_description,
    acronyms_found       = .env$acronyms_found,
    num_expanded_tokens  = .env$num_expanded_tokens,
    mean_local_score     = .env$mean_local_score,
    best_local_score_any = .env$best_local_score_any,
    ambiguity_flag       = .env$ambiguity_flag
  ) %>%
  dplyr::select(-.row_id, -raw_text)
