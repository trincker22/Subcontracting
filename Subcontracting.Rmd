...
overall %>%
  ...
  labs(title = "YoY Change: Number of Contracts (Overall)", x = "Year", y = "YoY %") +
  theme_clean
...
overall %>%
  filter(!is.na(yoy_val)) %>%
  ggplot(aes(x = year, y = yoy_val)) +
  geom_line() + geom_point() +
  scale_y_continuous(labels = label_percent(scale = 0.01)) +
  scale_x_continuous(breaks = sort(unique(overall$year))) +
  labs(title = "YoY Change: Total Contract Value (Overall)", x = "Year", y = "YoY %") +
  theme_clean
...
monthly_series <- sub_prep %>%
  mutate(month = floor_date(prime_date, "month")) %>%
  group_by(month) %>%
